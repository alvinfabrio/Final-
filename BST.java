...
public class BST<T extends Comparable<T>> {
	...
	public T getRoot() throws NoSuchElementException {
		if (isEmpty()) {
			throw new NoSuchElementException("getRoot(): Tree is Empty! Cannot get root!");
		}
		return root.data;
	}
	...
	public int getHeight() {
		return getHeight(root); // Seems correct?
	}
...
/**FIX SEARCH**/	
	 ...
	 */
	public void insert(T data) { // Code given in Lesson 10
		if (root == null) {
			root = new Node(data);
		} else {
			insert(data, root);
	...
	private void insert(T data, Node node) {
		if (node.data.compareTo(data) > 0 || node.data.equals(data)) { // Check if value to insert is ≤ to data stored
																		// at node.
			if (node.left == null) {
	...
	public void remove(T data) throws NoSuchElementException {
		if(this.getSize()==0) {
			throw new NoSuchElementException("remove(T data): Cannot remove from an empty tree!");
		}
		else if(this.search(data)==false) {
			throw new NoSuchElementException("remove(T data): Node does not exist in this tree!");
		}
		else {
	...
	public void arraytransfer(BST b, Hotel[]arr ) {
		if(b.root==null) {
