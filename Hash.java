...
public class Hash<T extends Comparable<T>> {
	...
	public int countBucket(int index) throws IndexOutOfBoundsException {
		if (index < 0 || index >= Table.size()) {
			throw new IndexOutOfBoundsException("countBucket(int index): Invalid index!");
		}
	...
	public int search(T t) {
		Table.get(hash(t)).placeIterator();
		for (int i = 0; i < Table.get(hash(t)).getLength(); i++) {
			if (Table.get(hash(t)).getIterator().equals(t)) {
				return i;
			}
			Table.get(hash(t)).advanceIterator();
		}
		return -1;
	}
	...
	public void printBucket(int bucket) throws IndexOutOfBoundsException {
		int len = countBucket(bucket);
		if (len >= Table.size()) {
			throw new IndexOutOfBoundsException("printBucket(int bucket): Invalid bucket size.");

		} else if (len <= 0) {
			System.out.println("Bucket " + bucket + " is empty, nothing to print");
		} else {
		}
	...
	public String toString() {
		...
		for (int i = 0; i < Table.size(); i++) {
			if (!(Table.get(i).isEmpty())) {
				...
				for (int j = 1; j <= countBucket(i); j++) {
					s += Table.get(i).getIterator() + "\n\n";
